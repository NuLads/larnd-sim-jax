...
# SCAN2D="lifetime,Ab" -> joint 2-D grid instead of independent 1-D sweeps. Minimising over the
# second parameter at each value of the first gives the PROFILE likelihood in the first, which is
# what a displaced minimum must be measured on. Cost is steps^2 x nbatch, so keep STEPS modest.
export LARND_SCAN_2D=${SCAN2D:-}
...
python3 -m optimize.example_run \
    ...
    --loss_fn llhd ${LOSSSIGQ:+--loss_sigma_charge ${LOSSSIGQ}} \
    --probabilistic_sim \
