...
#SBATCH --job-name=pos_only
#SBATCH --output=/sdf/group/neutrino/pgranger/larnd-sim-jax/logs/pos_only/job-%A_%a.out
#SBATCH --ntasks=1
...
MCS_ARR=(0.5 0.1 0.02)
CLR_ARR=(1e-4 5e-4 1e-3)
...
LABEL=posonly_frozen_mcs${MCS_PRIOR_WEIGHT}_clr${CHAIN_LR}_${ITERATIONS}iter
TEST_NAME=pos_only

mkdir -p /sdf/group/neutrino/pgranger/larnd-sim-jax/logs/pos_only
