...
#SBATCH --job-name=scan_bs2
#SBATCH --output=/sdf/group/neutrino/pgranger/larnd-sim-jax/logs/scan_bs2/job-%A.out
#SBATCH --ntasks=1
...
mkdir -p /sdf/group/neutrino/pgranger/larnd-sim-jax/logs/scan_bs2
SIF=/sdf/group/neutrino/pgranger/larnd-sim-jax/larndsim-jax_main.sif
echo "=== GPU spec ==="; nvidia-smi --query-gpu=name,memory.total --format=csv,noheader

run_one () {
  local blen="$1"
  echo "================ max_batch_len = ${blen} cm ================"
  ( peak=0; for k in $(seq 1 400); do
      m=$(nvidia-smi --query-gpu=memory.used --format=csv,noheader,nounits 2>/dev/null | head -1)
      [ -n "$m" ] && [ "$m" -gt "$peak" ] && peak=$m; echo "$peak" > /tmp/pk2_${blen}.txt; sleep 1
    done ) & SAMP=$!
