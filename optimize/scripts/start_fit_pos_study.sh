...
#SBATCH --job-name=pos_study
#SBATCH --output=/sdf/group/neutrino/pgranger/larnd-sim-jax/logs/pos_study/job-%A_%a.out
#SBATCH --ntasks=1
...
PARAM=long_diff
# Scan chain-fit aggressiveness: (mcs_prior_weight, chain_lr)
MCS_ARR=(0.5 0.1 0.5 0.1 0.02 0.5)
CLR_ARR=(1e-4 1e-4 5e-4 5e-4 1e-3 1e-3)
MCS_PRIOR_WEIGHT=${MCS_ARR[$SLURM_ARRAY_TASK_ID]}
...
LABEL=pos_${PARAM}_mcs${MCS_PRIOR_WEIGHT}_clr${CHAIN_LR}_${ITERATIONS}iter
TEST_NAME=pos_study

mkdir -p /sdf/group/neutrino/pgranger/larnd-sim-jax/logs/pos_study

echo "=== Job started: $(date) ==="
echo "=== long_diff + chain (mcs=${MCS_PRIOR_WEIGHT} clr=${CHAIN_LR}) ==="
