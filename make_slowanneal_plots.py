...
runs = {}
for f in sorted(glob.glob('fit_result/sci_full_SLOWANNEAL/history_iter*.pkl')):
...
lr = np.array([float(optax.warmup_exponential_decay_schedule(
    ...
    decay_rate=0.9539, staircase=True)(t)) for t in range(N)])
...
for ax, p in zip(axes.ravel(), P):
    ...
    ax.axvline(8000, ls=':', color=RED, lw=1.2)
...
ax.axvline(8000, ls=':', color=RED, lw=1.2)
ax.text(8150, 30, 'LR < 0.1% of peak', fontsize=7.5, color=RED)
...
fig.suptitle('SLOWANNEAL — parameter traces, 3 seeds x 10000 iterations. Thin = raw, thick = rolling median.\n'
             'Shaded ±5%. decay 0.9539 over 10k = same FINAL LR as ANNEAL but 1.78x more total travel.',
             x=.02, ha='left', y=1.02)
...
fig.suptitle('SLOWANNEAL — convergence diagnostics. (a) is the one to read: it shows how the reported '
...
axes[0].axvline(5000, ls=':', color=RED)
axes[0].set_title('Position residual', loc='left', fontsize=9)
...
fig.suptitle('SLOWANNEAL — quantities that had NOT converged at 5000 iterations', x=.02, ha='left', y=1.04)
fig.tight_layout(); fig.savefig(f'{OUT}/C_position_dedx_loss.png'); plt.close(fig)

# numeric companion
print('SLOWANNEAL convergence summary (robust: trailing-20% median)')
